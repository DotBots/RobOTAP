 ...
 * @ingroup     drv
 * @brief       Firmware update library either over the air (OTA) or via
 * Bootloader
 *
 * @{
 * @file
...
typedef void (*db_ota_reply_t)(const uint8_t *,
                               size_t);  ///< Transport agnostic function used to
                                         ///< reply to the flasher script
...
typedef struct {
    db_ota_reply_t
        reply;           ///< Pointer to the function used to reply to the flasher script
...
typedef struct __attribute__((packed, aligned(4))) {
    ...
    uint8_t
        signature[DB_OTA_SIGNATURE_LENGTH];  ///< Signature of the firmware hash
#endif
