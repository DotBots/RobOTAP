...
void db_ota_start(void) {
    ...
    _ota_vars.addr =
        _ota_vars.table.partitions[_ota_vars.target_partition].address;
}
...
void db_ota_handle_message(const uint8_t *message) {
    ...
    switch (message_type) {
        case DB_OTA_MESSAGE_TYPE_INFO:
        {
            ...
            memcpy(&_ota_vars.reply_buffer[1], &message_info,
                   sizeof(db_ota_message_info_t));
            _ota_vars.config->reply(_ota_vars.reply_buffer,
                                    sizeof(db_ota_message_type_t) +
                                        sizeof(db_ota_message_info_t));
        } break;
        case DB_OTA_MESSAGE_TYPE_START:
        {
            const db_ota_start_notification_t *ota_start =
                (const db_ota_start_notification_t *)&message[1];
#if defined(OTA_USE_CRYPTO)
            const uint8_t *hash = ota_start->hash;
            if (!crypto_ed25519_verify(ota_start->signature, DB_OTA_SIGNATURE_LENGTH,
                                       (const uint8_t *)ota_start,
                                       sizeof(db_ota_start_notification_t) -
        ...
        case DB_OTA_MESSAGE_TYPE_FW:
        {
...
#if defined(OTA_USE_CRYPTO)
                crypto_sha256_update(&_ota_vars.sha256_ctx,
                                     (const uint8_t *)ota_pkt->fw_chunk,
                                     DB_OTA_CHUNK_SIZE);
#endif
            }
            ...
            _ota_vars.config->reply(_ota_vars.reply_buffer,
                                    sizeof(db_ota_message_type_t) + sizeof(uint32_t));
