 ...
 * @brief This application can be flashed on partition 1 (address depends on
 * platform, see FLASH1 field in MemoryMap.xml)
 *
...
#ifdef DB_LED2_PIN
static void _toggle_led(void) { db_gpio_toggle(&db_led2); }
#endif
...
int main(void) {
  printf("Booting on partition %d (Build: %s)", DOTBOT_PARTITION,
         DOTBOT_BUILD_TIME);
...
#if defined(DB_LED1_PIN) && defined(DB_BTN1_PIN)
  db_gpio_init_irq(&db_btn1, DB_GPIO_IN_PU, DB_GPIO_IRQ_EDGE_RISING,
                   _btn_toggle_callback, NULL);
#endif
