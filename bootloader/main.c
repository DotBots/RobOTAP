...
static void _jump_to_image(uint32_t *address) {
  vector_table_t *table =
      (vector_table_t *)address; // Image should start with vector table
...
#ifdef DB_LED4_PIN
static void _blink_led4(void) { db_gpio_toggle(&db_led4); }
#endif
...
int main(void) {
  ...
  uint32_t active_image =
      (_bootloader_vars.table.active_image < DB_PARTITIONS_MAX_COUNT)
          ? _bootloader_vars.table.active_image
          : 0;
  ...
  if (keep_active) {
    ...
    db_uart_init(0, &db_uart_rx, &db_uart_tx, DB_UART_BAUDRATE,
                 &_uart_callback);
  ...
  _jump_to_image(
      (uint32_t *)_bootloader_vars.table.partitions[active_image].address);
}
